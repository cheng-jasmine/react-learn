...
export async function getAllStudents() {
    return await fetch("/api/student/findAll?appkey=" + appkey).then(resp => resp.json()).then(resp => resp.data)
}
...
export async function searchStudents({
    ...
    if (key) {
        const resp = await fetch(`/api/student/searchStudent?appkey=${appkey}&page=${page}&size=${limit}&key=${key}&sex=${sex}`).then(resp => resp.json()).then(resp => resp.data)
